...
export default function WeddingDetailsHeader({ eventType }: { eventType?: string }) {
  const title = eventType === "homecoming" ? "Homecoming Details" : "Wedding Details";
  const message = eventType === "homecoming" 
    ? "All the important information you need to celebrate our homecoming with us."
    : "All the important information you need to celebrate our special day with us.";

  return (
    <section className={`w-full ${eventType === "homecoming" ? "bg-[#faf9f6]" : "bg-white"} pt-24 pb-16 px-4 flex flex-col items-center text-center relative z-10`}>
      <motion.div 
