  ...
  return (
    <motion.div
      ...
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-colors duration-500 ${
        isHomecoming ? "bg-blush" : "bg-sand-light"
      }`}
    >
      <div className={`relative w-full max-w-sm sm:max-w-md aspect-[3/5] shadow-2xl rounded-sm overflow-hidden flex flex-col items-center justify-center text-charcoal text-center px-8 border transition-all duration-500 ${
        isHomecoming ? "bg-white border-burgundy/10 shadow-burgundy/5" : "bg-sand border-black/5"
      }`}>
        
        {/* Top Left Decorative Pattern Placeholders (can be replaced by images later) */}
        <div className={`absolute top-0 left-0 w-32 h-32 opacity-60 transition-colors duration-500 ${
          isHomecoming ? "text-rose-gold/40" : "text-gold-light"
        }`}>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full w-full py-12">
          ...
          <div className="flex-1 flex flex-col items-center justify-start mt-10">
            ...
            <h3 className={`text-2xl sm:text-3xl font-serif font-bold text-charcoal mb-6 border-b pb-2 min-w-[120px] transition-colors duration-500 ${
              isHomecoming ? "border-burgundy/25" : "border-gold/30"
            }`}>
              {guestName}
            </h3>
            ...
            <div className={`relative w-28 h-28 sm:w-32 sm:h-32 rounded-full overflow-hidden shadow-lg border-2 mb-2 transition-colors duration-500 ${
              isHomecoming ? "border-blush-dark shadow-burgundy/10" : "border-white"
            }`}>
              <Image 
                src="/couple.png" 
          ...
          <div className="flex-1 flex flex-col items-center justify-end pb-8">
            <button
              onClick={onOpen}
              className={`px-8 py-4 rounded-full text-white text-xs sm:text-sm font-bold uppercase tracking-[0.25em] transition-all duration-300 active:scale-95 shadow-md ${
                isHomecoming 
        ...
        <div className={`absolute bottom-0 right-0 w-32 h-32 opacity-60 transform rotate-180 transition-colors duration-500 ${
          isHomecoming ? "text-rose-gold/40" : "text-gold-light"
        }`}>
          <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 0C55.2285 0 100 44.7715 100 100H0V0Z" fill="currentColor" opacity="0.3"/>
