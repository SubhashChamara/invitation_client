...
export default function HomecomingThankYouPage() {
  const [imageSrc, setImageSrc] = useState<string | null>(null);
  const [imageError, setImageError] = useState(false);
  const [imageLoading, setImageLoading] = useState(true);

  const shortDate = formatShortDate(HOMECOMING_DATE_ISO);

  useEffect(() => {
    const controller = new AbortController();
    // Explicitly request the homecoming type
    fetch(API_BASE_URL + "/api/thanks-image?type=homecoming", { signal: controller.signal })
      .then((res) => {
